...
export function DataTable({ data }: { data: Record<string, unknown>[] }) {
  ...
  return (
    <div className="space-y-2 overflow-x-auto">
      ...
      />
      <table className="responsive-table w-full min-w-[20rem] border border-[var(--border)] text-sm">
        <thead>
        ...
        <tbody>
          {filtered.map((row, i) => (
            <tr key={i} className="motion-row border-t border-[var(--border)]">
              {Object.entries(row).map(([key, value], j) => (
                <td key={j} className="p-2" data-label={key}>
                  {String(value)}
                </td>
              ))}
            </tr>
